    ...
    if (ownership) {
      switch (ownership) {
        ...
        case 'duplicate':
          ...
          break;
        case 'you_need':
          // Apenas cromos repetidos do outro usuário (quantity > 1) que me faltam (quantity = 0)
          if (isExternal) {
            filteredStickers = filteredStickers.filter(s => s.youNeed);
          }
          break;
        case 'you_have':
          // Apenas cromos repetidos que eu possuo (quantity > 1) que faltam para o outro usuário (quantity = 0)
          if (isExternal) {
